...
var isMatch = function(value, oldValue, fields) {
    var fields = Array.isArray(fields) ? fields : [fields];
    var trues = 0;
    
    fields.forEach(function(field){
        if(field in value && value[field] && field in oldValue && oldValue[field]) {
            if(value[field] === oldValue[field]) {
                trues++;
            } else if(typeof value[field].equals === "function" && value[field].equals(oldValue[field])) {
                trues++;
            }
        }
    });
    
    return (trues === fields.length);
};

var mergeArray = function(self, path, values, opts) {
    var schema = self.schema.path(path);
    var fields = schema.options.mergeidentifier || MERGE_IDENTIFIER_DEFAULT;
    ...
    values.forEach(function(value){
        ...
        oldValues.forEach(function(oldValue, oldIndex){
            if(isMatch(value, oldValue, fields)) {
    ...
    self.schema.eachPath(function(path, pathOptions) {
        if (MERGE_FIELD_FILTER.indexOf(path) === -1 &&
            ...
            if (value !== undefined) {
                ...
                } else if(!override && Array.isArray(value) && value.length > 0){
                    value.forEach(function(val){
                        self[path].addToSet(val);
                    });
                } else {
                    self.set(path, value);
                }
